...
작업이 쌓였을 때 모든 일을 단일 모델로 처리하면 두 가지 문제가 발생합니다.

- **고성능 모델 전담**: 단순 오타나 포맷팅 같은 작업에 비싼 모델을 쓰게 되어 비용이 낭비됩니다.
- **경량 모델 전담**: 미묘하고 복잡한 로직을 경량 모델에 맡기면, 오류를 제대로 잡지 못하고
  코드를 조용히 망가뜨립니다.

결국 "어떤 모델에 맡길 것인가"에 대한 판단이 매번 직관에 의존해 일관성 없이 내려지게 됩니다.

이 스킬은 단순한 모델 추천에 그치지 않습니다. 메인 세션이 작업 등급을 판단·그룹화·검증하고,
...
**판단 원칙**: 등급이 애매하다면 상위 모델로 올려서 배정합니다. 모델 사용 비용보다 잘못
수정된 코드를 복구하는 비용이 훨씬 크기 때문입니다.

### 핵심은 "위임하지 않을 조건"을 구별하는 것

이 스킬의 핵심은 역설적으로 "어떤 작업을 위임하지 않을 것인가"에 있습니다.

서브에이전트는 이전 대화 맥락을 전혀 모른 채 작업에 투입됩니다. 맥락이 부족한 에이전트는
작업이 막혔을 때 멈추는 게 아니라, 그럴듯한 환각(hallucination)을 만들어내며 엉뚱한 코드를
수정하곤 합니다. 이로 인한 부작용은 사고가 터지기 전까지 눈에 띄지 않습니다.

따라서 작업을 서브에이전트에 넘기기 전, 다음 조건들을 먼저 검증합니다.

- **작업의 규모** — 1~2개 수준의 소규모 작업이거나 상호 의존성이 높은 작업은 메인 세션이
  직접 처리하는 게 빠릅니다.
- **자기완결성** — "아까 말한 그것"처럼 이전 맥락이 필요한 요청은 위임할 수 없습니다. 요청을
  완전히 구체화한 뒤 넘기거나, 직접 처리해야 합니다.
- **그룹화 후 최종 그룹 수** — 동일한 파일을 수정하는 작업들은 하나의 호출로 묶어야 합니다.
  예를 들어 10개의 작업이 결과적으로 CSS 파일 하나로 모인다면 최종 그룹은 1개입니다. 이
  경우 병렬 처리가 불가능하므로 컨텍스트 절약 외에는 이득이 없고, 메인 세션이 이미 그
  파일을 읽었다면 그 이득조차 없습니다. **핵심 기준은 개별 항목 수가 아니라 묶인 뒤의 최종
  그룹 수(2개 이상)입니다.**
- **분할 불가능한 고난도 작업** — 억지로 쪼개지 않고, 메인 세션의 모델 자체를 전환하도록 한
  줄로 추천합니다.

### 검증 및 예외 처리

서브에이전트의 "완료했습니다"라는 응답을 그대로 신뢰하지 않습니다.

1. **단계별 검증** — 등급에 따라 검증 수준을 차등 적용합니다(눈으로 확인 / 테스트 실행 /
   diff 정독).
2. **단계적 승격** — 작업이 실패하면 한 등급 높은 모델로 재시도합니다.
3. **무한 재시도 방지** — 그래도 계속 실패하면 무한 루프에 빠지지 않고 즉시 중단한 뒤
   사용자에게 보고합니다.
4. **커밋 제한** — 자동 커밋은 절대 수행하지 않습니다.
